...
interface GenerateThreadResult {
  threadId: string;
...
export async function generateThread(
  ...
  try {
    ...
    const defaultCharLimit = platform === 'linkedin' ? 500 : 280;
    const effectiveCharLimit = characterLimit || defaultCharLimit;

    // Call the edge function with thread mode
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/generate-post`,
      {
        ...
        body: JSON.stringify({
          ...
          characterLimit: effectiveCharLimit,
          mode: 'thread',
          threadConfig: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 402 || errorData.code === 'INSUFFICIENT_CREDITS') {
        return '❌ Insufficient credits! You need at least 1 credit to generate a thread.\n\nBuy credits at: https://postidentity.com/credits';
      }
      
      throw new Error(errorData.error || `HTTP ${response.status}`);
    }

    const result = await response.json() as GenerateThreadResult;

    if (!result.threadPosts || result.threadPosts.length === 0) {
      throw new Error('No thread generated');
    }

    // Format output
    let output = '✅ Thread generated successfully!\n\n';
    output += `📝 ${result.threadPosts.length} posts in thread\n`;
    output += '─'.repeat(50) + '\n\n';
    
    result.threadPosts.forEach((post, idx) => {
      output += `**Post ${idx + 1}/${result.threadPosts.length}** (${post.content.length} chars)\n`;
      output += post.content + '\n';
      output += '─'.repeat(30) + '\n\n';
    });
