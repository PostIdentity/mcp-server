...
export async function listIdentities(): Promise<string> {
  try {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/profiles?user_id=eq.${userId}&archived_at=is.null&select=id,name,description,created_at&order=created_at.desc`,
      {
        headers: {
    ...
    const data = await response.json() as Array<{ id: string; name: string; description: string | null; created_at: string }>;
