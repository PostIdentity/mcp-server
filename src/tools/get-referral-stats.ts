...
export async function getReferralStats(): Promise<string> {
  try {
    ...
    const codeResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/user_preferences?user_id=eq.${userId}&select=referral_code`,
      {
        headers: {
    ...
    const codeData = await codeResponse.json() as Array<{ referral_code: string | null }>;
    ...
    const statsResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/rpc/get_referral_stats`,
      {
        method: 'POST',
        headers: {
          'apikey': SUPABASE_ANON_KEY,
    ...
    const stats = await statsResponse.json() as { total_referrals: number; credits_earned: number };
