...
export async function createIdentity(
  ...
  try {
    ...
    const generateResponse = await fetch(
      `${SUPABASE_URL}/functions/v1/generate-profile`,
      {
        method: 'POST',
        headers: {
    ...
    if (!generateResponse.ok) {
      const errorData = await generateResponse.json().catch(() => ({ error: 'Unknown error' })) as { error?: string };
      throw new Error(errorData.error || `HTTP ${generateResponse.status}`);
    }

    const profileData = await generateResponse.json() as any;
    ...
    const createdProfile = await createResponse.json() as Array<{ id: string; name: string }>;
