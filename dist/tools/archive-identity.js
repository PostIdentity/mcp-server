...
export async function archiveIdentity(identityId) {
    try {
        const userId = getUserId();
        const accessToken = getAccessToken();
        // First verify the identity belongs to the user
        const verifyResponse = await fetch(`${SUPABASE_URL}/rest/v1/profiles?id=eq.${identityId}&user_id=eq.${userId}&select=name`, {
            headers: {
        ...
        const archiveResponse = await fetch(`${SUPABASE_URL}/rest/v1/profiles?id=eq.${identityId}`, {
            method: 'PATCH',
            headers: {
                'apikey': SUPABASE_ANON_KEY,
                'Authorization': `Bearer ${accessToken}`,
...
//# sourceMappingURL=archive-identity.js.map